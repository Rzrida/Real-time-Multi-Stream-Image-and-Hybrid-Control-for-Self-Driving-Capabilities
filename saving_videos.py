...
class MultiStreamProcessor:
    ...
    def _worker(self, path):
        fps_counter = FPSCounter()
        try:
            for pkg in process_video_week2(path, show=False, fps_counter=fps_counter):
...
def _process_and_save(vid_path: str, out_dir: Path, show: bool = True):
    vid_name = Path(vid_path).stem
    fps_counter = FPSCounter()

    writer_composite = None
    writer_mosaic    = None
    ...
    for idx, pkg in enumerate(process_video_week2(vid_path, show=show, fps_counter=fps_counter)):
        log_frame(pkg, idx)

        # Initialise writers on first frame (size only known then)
        if writer_composite is None:
            fourcc = cv2.VideoWriter_fourcc(*"mp4v")
            ch, cw = pkg["composite"].shape[:2]
            mh, mw = pkg["debug_mosaic"].shape[:2]
            writer_composite = cv2.VideoWriter(
                str(out_dir / f"{vid_name}_composite.mp4"), fourcc, 20, (cw, ch))
            writer_mosaic = cv2.VideoWriter(
                str(out_dir / f"{vid_name}_mosaic.mp4"),    fourcc, 20, (mw, mh))

        writer_composite.write(pkg["composite"])
        writer_mosaic.write(pkg["debug_mosaic"])

    if writer_composite: writer_composite.release()
    if writer_mosaic:    writer_mosaic.release()

    print(f"\n{'─'*60}")
    print(f"  Saved → {out_dir / f'{vid_name}_composite.mp4'}")
    print(f"  Saved → {out_dir / f'{vid_name}_mosaic.mp4'}")
    print(f"  Total frames : {fps_counter.total_frames}")
    print(f"  Elapsed      : {fps_counter.elapsed_time:.2f}s")
    print(f"  Average FPS  : {fps_counter.average_fps:.2f}")
    print(f"{'─'*60}")

def main():
    BASE = r"C:\Users\rzrid\Desktop\DIP\Project\Dataset"

    dataset_groups = {

        "simple_mydataset": [
            rf"{BASE}\simple_mydataset\vid1.mp4",
            rf"{BASE}\simple_mydataset\vid2.mp4",
            rf"{BASE}\simple_mydataset\vid3.mp4",
            rf"{BASE}\simple_mydataset\vid4.mp4",
        ],
        ...
        "multistream": [
            rf"{BASE}\MultiStreamDataset\Video1.mp4",
            rf"{BASE}\MultiStreamDataset\Video2.mp4",
            rf"{BASE}\MultiStreamDataset\Video3.mp4",
            rf"{BASE}\MultiStreamDataset\Video4.mp4",
        ],

        "original_videos": [
            rf"{BASE}\Videos\PXL_20250325_043754655.TS.mp4",
            rf"{BASE}\Videos\PXL_20250325_043922504.TS.mp4",
            rf"{BASE}\Videos\PXL_20250325_044505516.TS.mp4",
            rf"{BASE}\Videos\PXL_20250325_044603023.TS.mp4",
            rf"{BASE}\Videos\PXL_20250325_044746327.TS.mp4",
            rf"{BASE}\Videos\PXL_20250325_045117252.TS.mp4",
        ],
    ...
    for group_name in active_groups:
        videos = dataset_groups[group_name]

        out_dir = Path(BASE) / "output" / group_name
        out_dir.mkdir(parents=True, exist_ok=True)

        print(f"\n{'#'*60}")
        print(f"  GROUP: {group_name}  ({len(videos)} videos)")
        ...
        for vid_path in videos:
            if not Path(vid_path).exists():
                ...
                continue
            _process_and_save(vid_path, out_dir, show=True)

    print("\n\nAll groups processed and saved.")
